window.onload = () => {
    ...
    class Vec3 {
        ...
        render( xoff, yoff, Z ) {
            const scaler = Math.abs(Z) + this.z;
            ctx.beginPath();
            // Darker at distance
            //const shade = parseInt( 255 * ( 50 / ( Math.abs(Z) + this.z )) );
            //ctx.fillStyle = "rgb( "+shade+", 0, 0 )";
            //ctx.strokeStyle = "rgb( "+shade+", "+shade+", "+shade+" )";
            ctx.fillStyle = "black";
            ctx.strokeStyle = "black";
            ctx.arc( this.x + xoff, this.y + yoff, 125 / scaler, 0, 2 * Math.PI);
            ctx.fill();
            //ctx.stroke();
        }
    ...
    class Shape {
        ...
        render( x_offset, y_offset, Z ) {
            ...
            //this.vectors.map((vector) => vector.render( x_offset, y_offset, Z ) );
        }
    }
